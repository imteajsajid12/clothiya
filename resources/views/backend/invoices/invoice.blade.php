	...
	<div>
        ...
        <div style="padding: 2rem;padding-bottom: 0">
            <table>
                ...
                @endphp
                <tr><td class="strong small gry-color" style="font-size: 14px; font-weight:600;">{{ translate('Bill to') }}:</td></tr>
                <tr><td class="strong" style="font-size: 14px; font-weight:600;">{{ $shipping_address->name }}</td></tr>
                <tr><td class="gry-color small" style="font-size: 14px; font-weight:600;">{{ $shipping_address->address }}, {{ $shipping_address->city ?? null}},  @if(isset(json_decode($order->shipping_address)->state)) {{ json_decode($order->shipping_address)->state }} - @endif {{ $shipping_address->country }}</td></tr>
                        ...
                        <td>
                            <table class="text-right sm-padding small strong">
                                <tbody>
                                    ...
                                    @php
                                    $total = $order->orderDetails->sum('price') + $order->orderDetails->sum('tax') + $order->orderDetails->sum('shipping_cost') - $order->coupon_discount;
