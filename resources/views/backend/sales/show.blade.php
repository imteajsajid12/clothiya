...
@endif
<form action="{{ route('order.edit') }}" method="POST" id="edit_order_form">
    @csrf
    <div class="card">
        ...
        <div class="card-body">
            <div class="row gutters-5">
              ...
              {{-- edit order fiels --}}
              <div class="edit mt-4 d-none" id="edit">
                ...
                <div class="row">
                   ...
                   <div class="col-md-2 ml-auto">
                       <div class="form-group">
                           ...
                           <input type="text" class="form-control"  id="name" name="name" value="{{ json_decode($order->shipping_address)->name }}" placeholder="Enter Name">
                           <input type="hidden" hidden class="form-control" name="id" id="id" value="{{$order->id }}">
                         </div>
                   </div>
            <div class="col-md-2 ml-auto">
               <div class="form-group">
                   <label for="exampleFormControlInput1">phone</label>
                   <input type="number" class="form-control" name="phone" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==11) return false;" value="{{ json_decode($order->shipping_address)->phone }}" id="phone" placeholder="Enter phone">
           ...
           <div class="col-md-2 ml-auto">
               <div class="form-group">
                   <label for="exampleFormControlInput1">Shipping Cost</label>
                   <input type="number" class="form-control" id="shipping" name="shipping" value="{{  $order->orderDetails->sum('shipping_cost') }}"  placeholder="Enter Cost">
                </div>
           </div>
           ...
           <div class="col-md-2 ml-auto">
               <div class="form-group">
                   ...
                   <input type="number" class="form-control" id="discount" name="discount" value="{{  $order->discount }}"  placeholder="Enter Cost">
                </div>
           </div>
           <div class="col-md-2 ml-auto">
               <div class="form-group">
                   <label for="exampleFormControlInput1">Order Note</label>
                   <input type="text" class="form-control" id="order_note" name="order_note" value=" {{ json_decode($order->shipping_address)->note ?? null}}"  placeholder="Enter Note">
                </div>
           </div>
           <div class="col-md-2 ml-auto">
            <div class="form-group">
                ...
                <textarea type="text" class="form-control" name="address" id="address" placeholder="Enter Address ">
                    {{ json_decode($order->shipping_address)->address }}, {{ json_decode($order->shipping_address)->city ?? null}}, @if(isset(json_decode($order->shipping_address)->state)) {{ json_decode($order->shipping_address)->state }} - @endif {{ json_decode($order->shipping_address)->postal_code ?? null}}
                </textarea>
                </div>
        </div>
       </div>
...
<!-- Second select box (Initially hidden) -->
                ...
                <div class="col text-md-left text-center">
                    @if(json_decode($order->shipping_address))
                        <address>
                            ...
                            {{ json_decode($order->shipping_address)->address }}, {{ json_decode($order->shipping_address)->city ?? null }}, @if(isset(json_decode($order->shipping_address)->state)) {{ json_decode($order->shipping_address)->state }} - @endif {{ json_decode($order->shipping_address)->postal_code ?? null}}<br>
                    ...
                    <table>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    {{ translate($order->reference) ?? null }}</td>
                            </tr>
                            {{-- @if (json_decode($order->shipping_address)->note != null) --}}
                            <tr>
                                <td class="text-main text-bold">{{ translate('Order Note') }}</td>
                                <td class="text-right">
                                    {{ json_decode($order->shipping_address)->note ?? null}}</td>
                            </tr> 
                            {{-- @endif --}}
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-right">{{ json_decode($order->shipping_address)->user ?? null }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <div class="d-flex check_btn aiz-plus-minus ">
                                            <button
                                                ...
                                                onclick="plus_quantity({{ $orderDetail->id }}, {{ $orderDetail->quantity }})">
                                                <i class="las la-plus"></i>
                                            </button>
                                            <input type="number" name="quantity[]"
                                               id="edit-quantity{{ $orderDetail->id }}"
                                                class=" border-0  text-center quantity"
                                                placeholder="1"  value="{{ $orderDetail->quantity }}"
                                                min="1"
                                              >
                                              {{--<input type="text" name="quantity[{{ $orderDetail->id }}]" id="quantity{{ $orderDetail->id }}" class="d-none" value="{{ $orderDetail->quantity }}">--}}
                                            <button
                                                class="btn col-auto btn-icon btn-sm btn-circle btn-light"
                                                type="button"
                                                data-field="quantity[{{ $orderDetail->id   }}]  "  onclick="minus_quantity({{ $orderDetail->id }}, {{ $orderDetail->quantity }})">
            ...
            <div class="clearfix float-right">
                ...
                <div class="no-print text-right">
                    <button  type="button" id="update_order" class="btn btn-icon btn-success">
                        {{-- <i class="fa fa-refresh"></i> --}}
                        <i class="fa fa-check"></i>
                    </button>
                    <button type="button" id="edit_order" class="btn btn-icon btn-primary"><i
...
<script type="text/javascript">
    ...
    function minus_quantity(key , value) {
        let currentValue = parseInt(document.getElementById("edit-quantity" + key).value); 
        let newValue = currentValue - 1;
         document.getElementById("edit-quantity" + key).value = newValue;
    }
    function plus_quantity(key , value ) {
        let currentValue = parseInt(document.getElementById("edit-quantity" + key).value);
        let newValue = currentValue + 1;
         document.getElementById("edit-quantity" + key).value = newValue;
    }
    function updateQuantity(key ,value) {
        ...
        $.post('{{ route('order.quantity_edit') }}', {
            _token: AIZ.data.csrf,
            id: key,
            quantity: value,
        }, function(data) {
             console.log(data.data);
             document.getElementById("edit-quantity"+key).value = value;
             location.reload();
        });
    }
</script>
