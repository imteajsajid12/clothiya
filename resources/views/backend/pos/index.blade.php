            ...
            <div class="col-md">
                <div class="row gutters-5 mb-3">
                    ...
                    <div class="col-md-3 col-6">
                        <select id="selectCategory" name="poscategory" class="form-control form-control-lg aiz-selectpicker" data-live-search="true" onchange="filterProducts()">
                    ...
                    {{-- subcategory --}}
                    <div class="col-md-3 col-6">
                        <select id="selectSubCategory" name="subcategory" class="form-control form-control-lg" data-live-search="true" onchange="filterProducts()">
                            <option value=''>{{ translate('Sub Categories') }}</option>
                        </select>
                    </div>
                    {{-- subcategory --}}
            ...
            <div class="col-md-auto w-md-350px w-lg-400px w-xl-500px">
                <div class="card mb-3">
                    <div class="card-body">
                        <div class="d-flex border-bottom pb-3">
                            ...
                            <button type="button" class="btn btn-icon ml-3 mr-0" data-target="#new-customer" data-toggle="modal" style="background:red; color:white;">
                ...
                <div class="pos-footer mar-btm">
                    <div class="d-flex flex-column flex-md-row justify-content-between">
                        <div class="d-flex">
                            <div class="dropdown mr-2 ml-0 dropup">
                            ...
                            {{-- reference --}}
                            <div class="ml-0 mr-1">
                                <select name="reference" class="form-control form-control-sm" required>
                                    <option value="">{{ translate('Reference') }}</option>
                                    <option value="Clothea">{{ translate('Clothea') }}</option>
                                    <option value="Teedress ">{{ translate('Teedress ') }}</option>
                                    <option value="Fancywear">{{ translate('Fancywear') }}</option>
                                    <option value="WhatsApp">{{ translate('WhatsApp') }}</option>
                                    <option value="Instagram">{{ translate('Instagram') }}</option>
                                    <option value="Reselling">{{ translate('Reselling') }}</option>
...
@section('script')
    ...
    $('#selectCategory').change(function(){
            ...
            var category_id = category_value.match(/\d+/)[0];
            if(category_id){
                $.ajax({
                type:"GET",
                url:"{{ url('/admin/pos/subcategory') }}/"+category_id,
                success:function(res){
                    if(res){
                        console.log(res);
                        $("#selectSubCategory").empty();
                        $("#selectSubCategory").append("<option value=''>{{ translate('Sub Categories') }}</option>");
                        $.each(res,function(key,value){
                            $("#selectSubCategory").append('<option value="category-'+value.id+'">'+value.name+'</option>');
                        });
                    }else{
                    $("#selectSubCategory").empty();
                    }
    ...
    // category code
        ...
        function filterProducts(){
            ...
            $.get('{{ route('pos.search_product') }}',{keyword:keyword, category:category, brand:brand, subcategory:subcategory}, function(data){
                products = data;
        ...
        function loadMoreProduct(){
            ...
            if(products != null && products.links.next != null){
                ...
                $.get(products.links.next,{keyword:keyword, category:category, brand:brand, subcategory:subcategory}, function(data){
                    products = data;
                    setProductList(data);
                });
            }
        ...
        function removeFromCart(id){
            $.post('{{ route('pos.removeFromCart') }}', {_token:AIZ.data.csrf, id:id}, function(data){
                updateCart(data);
                AIZ.plugins.notify('danger', "Product seleted successfully");
            });
        }
        ...
        function orderConfirmation(){
            ...
            if(reference.trim()==''){
                AIZ.plugins.notify('danger', "Please select Reference.");
            }else{
