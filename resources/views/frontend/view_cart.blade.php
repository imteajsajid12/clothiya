...
.col-xl-3 {
  ...
  .aiz-megabox > input:checked ~ .aiz-megabox-elem::before {
    content: '';
}  
</style>
    <section>
                  ...
                  {{-- <form action="{{ route('checkout.apply_coupon_code') }}" method="post">
                    <div class="row">
                        ...
                        <div class="col-12">
                            ...
                            <div class="mb-3 d-flex align-items-cente justify-content-between">
                                <input type="text" class="form-control mr-2" name="code" id="code" value="{{ old('code') }}" required placeholder="coupon code">
                                <button type="submit" class="btn btn-primary">Apply</button>
                            </div>
                        </div>
                    </div>
                  </form> --}}
                  <form class="form-default" data-toggle="validator" id="checkout-form" action="{{ route('checkout.without_auth') }}" role="form" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-12">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="myShipping" data-live-search="true" onchange="getShippingValue()" name="shipping_id" required>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <label for="code" class="form-label">কূপন</label>
                            <div class="mb-3 d-flex align-items-cente justify-content-between">
                                <input type="text" class="form-control mr-2" id="cuponCode" value="{{ old('code') }}" placeholder="Enter Discount Code">
                                <button type="button" id="new_cupon" class="btn btn-primary">Apply</button>
                            </div>
                        </div>
                    </div>
                    {{-- new cupon --}}
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function getShippingValue(){
            var selectElement = document.getElementById("myShipping");
            var selectedValue = selectElement.value;

            if (selectedValue) {
                var url = '{{ url('shipping/charge') }}' + '/' + selectedValue;
        ...
        $(document).ready(function(){
            $('#new_cupon').click(function () {
               ...
               if (cuponCode != '') {
                    ...
                    AIZ.plugins.notify('warning', "{{ translate('Please enter cupon code!') }}");
               }
            });
        });
        // new cupon code 
    ...
    ecommerce: {
        currency: "BDT",
      value: {{ $final_total }},
      items : [@foreach($carts as $key => $cartItem){
        @php
            $product = get_single_product($cartItem['product_id']);
            $product_stock = $product->stocks->where('variant', $cartItem['variation'])->first();
            // $total = $total + ($cartItem['price']  + $cartItem['tax']) * $cartItem['quantity'];
        ...
        @endphp
                ...
                item_name    : "{{ $product_name_with_choice }}",
                item_id      : "{{ $product->id }}",
                price        : "{{ cart_product_price($cartItem, $product, false) }}",
                item_brand   : "{{ $product->brands->name ?? 'N/A' }}",
                item_category: "{{ $product->main_category->name }}",
                item_variant : " {{ $cartItem['variation'] }}",
...
$(document).ready(function() {
 // Attach a change event handler to each input field
