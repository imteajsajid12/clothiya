                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <table class="table fs-14 text-soft-dark">
                                    ...
                                    <tr>
                                        <td class="w-50 fw-600 border-top-0 pl-0 py-2">{{ translate('Shipping Area')}}:</td>
                                        <td class="border-top-0 py-2">{{ json_decode($first_order->shipping_address)->shipping ?? null}}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="border-top-0 py-2">{{ json_decode($first_order->shipping_address)->address }}, {{ json_decode($first_order->shipping_address)->city ?? null }}, {{ json_decode($first_order->shipping_address)->country }}</td>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-md-6">
...
dataLayer.push({
  ...
  ecommerce: {
    ...
    value:"{{ single_price($combined_order->grand_total) }}",
    tax: "{{ single_price($combined_order->tax) }}",
    shipping: "{{ single_price($order->orderDetails->sum('shipping_cost')) }}",
    currency: "BDT",
    coupon: "{{ $order->coupon_discount == 0 ? translate('No Coupon') : translate('Coupon')  }}",

    items : [@foreach($order->orderDetails as $key => $orderDetail){
        ...
        index: 0,
        item_brand: "{{ $orderDetail->product->brand->name ?? 'NULL' }}",
        item_category: "{{ $orderDetail->product->category->name ?? 'NULL' }}",
        item_list_id: "NULL",
        item_list_name: "NULL",
        item_variant: "{{ $orderDetail->variation }}",
        location_id: "{{ json_decode($first_order->shipping_address)->address ?? null}}, {{ json_decode($first_order->shipping_address)->city ?? null}}, {{ json_decode($first_order->shipping_address)->country }}",
