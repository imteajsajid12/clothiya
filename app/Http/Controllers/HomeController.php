...
//        return view('frontend.' . get_setting('homepage_select') . '.partials.newest_products_section', compact('newest_products'));
        return view('frontend.minima.partials.newest_products_section', compact('newest_products'));
    }
    ...
    {
        return view('frontend.minima.partials.best_selling_section');
//        return ('frontend.' . get_setting('homepage_select') . '.partials.best_selling_section');
...
//        return view('frontend.minima.partials.best_selling_section');
    ...
    public function product(Request $request, $slug)
    {
        ...
        if ($detailedProduct != null && $detailedProduct->published) {
            ...
            // $sizeChartId = $detailedProduct->size_chart ?? ($detailedProduct->main_category && $detailedProduct->main_category->sizeChart) ? $detailedProduct->main_category->sizeChart->id : 0;  ;
            // $sizeChartName = ($detailedProduct->main_category && $detailedProduct->main_category->sizeChart) ? $detailedProduct->main_category->sizeChart->name : null;
            ...
            $size_chart = SizeChart::find($detailedProduct->size_chart ?? $sizeChartId );
            if ($size_chart) {
               
            
            $measurement_options = json_decode($size_chart->measurement_option) ?: 0 ;
            $measurement_option_inch = in_array("inch", $measurement_options) ? 1 : 0  ?? 0;
            $measurement_option_cen = in_array("cen", $measurement_options) ? 1 : 0 ?? 0;
    
            $measurementPoints = MeasurementPoint::whereIn('id', json_decode($size_chart->measurement_points, true))->get() ?? array();
            $size_options = AttributeValue::selectRaw('id,value')->whereIn('id', json_decode($size_chart->size_options, true))->get() ?? array();
            ...
            'measurementPoints', 'size_options', 'measurement_option_inch', 'measurement_option_cen', 'size_chart', 'data'
        ));
        }
    ...
    public function landing_page_details($id){
        $landingPage = LandingPage::findOrFail($id);
        $landing_product = LandingPageProduct::with('product:id,name,unit_price,current_stock,discount,choice_options,colors,photos')->where('landing_page_id',$id)->get();
...
// dd($landing_product);
        ...
        return view('frontend.landing_page_details',compact('landingPage','landing_product'));
    }
}
